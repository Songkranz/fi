...
local function clickButton(getButtonFn, timeout)
  ...
  -- BUG FIX: pcall เดิมคืน true เสมอแม้ไม่มี firesignal/getconnections
  -- ทำให้ปุ่มไม่โดนกด แต่โค้ดคิดว่ากดแล้ว
  local fired = false
  pcall(function()
    if firesignal then
  ...
  -- Fallback: ใช้ Activate() และ MouseButton1Click:Fire() ถ้า executor ไม่มี method พิเศษ
  if not fired then
...
-- BUG FIX: เพิ่มจาก 0.5 → 1.5 วิ ให้ MenuGUI โหลดทันก่อนหา Play button
task.wait(1.5)

-- กด Play — ลอง 3 รอบ กันเคสที่ clickButton ยิงแล้วปุ่มยังไม่ตอบสนอง
for _attempt = 1, 3 do
  local found = clickButton(function()
    local menu = PlayerGui:FindFirstChild("MenuGUI")
    if not menu then return nil end
    -- รองรับทั้ง MenuGUI.Play และ MenuGUI.Frame.Play
    return menu:FindFirstChild("Play", true)
  end, 8)
...
-- ตำแหน่ง Safe Zone (Position, ไม่ใช้ rotation)
Auto.SAFE_ZONE_POS      = Vector3.new(202, 43, -958)
...
--------------------------------------------------------------------
Auto.useLock            = false
Auto.useLockOwner       = nil -- BUG FIX: track thread ที่ถือ lock (ใช้ตรวจ deadlock)

function Auto:AcquireUse(stillOn, timeout)
  timeout = timeout or 15
  local t = os.clock()
  while self.useLock do
    -- BUG FIX: ถ้า owner thread ตายแล้วแต่ lock ยังค้าง (เช่น KillThread ไม่ได้ release)
    -- ให้ force-release แทนที่จะ timeout แล้ว return false ซึ่งทำให้ reroll หยุด
    if self.useLockOwner and coroutine.status(self.useLockOwner) == "dead" then
      warn("[Auto] lock owner ตายแล้วแต่ lock ยังค้าง — force release")
      self.useLock      = false
      self.useLockOwner = nil
      break
    end
    if stillOn and not stillOn() then return false end
    if (os.clock() - t) > timeout then
      warn("[Auto] AcquireUse timeout 15 วิ — force release เพื่อกัน deadlock")
      self.useLock      = false
      self.useLockOwner = nil
      break
    end
  ...
  self.useLock      = true
  self.useLockOwner = coroutine.running()
  return true
end
...
function Auto:KillThread(name)
  if self.threads[name] then
    -- BUG FIX: ถ้า thread นี้กำลังถือ useLock อยู่ ต้อง release ก่อน cancel
    -- task.cancel หยุด thread ทันที ไม่รัน ReleaseUse() → lock ค้าง → reroll หยุด
    if self.useLockOwner == self.threads[name] then
      self.useLock      = false
      self.useLockOwner = nil
    end
    task.cancel(self.threads[name])
...
function Auto:RerollAttributeLoop()
  ...
  while stillOn() do
    ...
    -- BUG FIX: refresh char หลัง AcquireUse — อาจรอนานจนตัวละคร respawn ใหม่
    -- char เดิมจะ stale ทำให้ equip บน character เก่า (destroyed) ใช้ไม่ได้
    char = LocalPlayer.Character
    if not char then
      self:ReleaseUse()
      continue
...
function Auto:DoubleOrbLoop()
  ...
  while stillOn() do
    ...
    -- BUG FIX: refresh char หลัง AcquireUse
    -- ถ้า player respawn ระหว่างรอ lock, char เก่าจะ stale
    -- ทำให้ equip 2x Orb บน character เก่า (destroyed) แล้ว UseItem ยิงแต่ server
    -- ไม่เห็น orb อยู่บนตัวจริง → 2x Orb ไม่โดน consume แต่ยังค้างบนตัว
    char = LocalPlayer.Character
    if not char then
      self:ReleaseUse()
      task.wait(0.5)
    ...
    -- BUG FIX: คืน orb กลับกระเป๋า — ใช้ char ที่ refresh แล้ว ไม่ใช่ char เก่า
    -- char เดิมอาจ stale (respawned) ทำให้ orb.Parent == char เป็น false
    -- orb จึงค้างบน character ใหม่โดยไม่ถูก return กลับ backpack
    do
      local currentChar = LocalPlayer.Character
      local bp = LocalPlayer:FindFirstChild("Backpack")
      if bp then
        -- เช็คทั้ง char ปัจจุบันและ char เดิม (กันเคส respawn ระหว่างรอบัฟ)
        if orb.Parent == currentChar or orb.Parent == char then
...
--==================================================================
-- GO TO SAFE ZONE
--   teleport ค้างที่ safe zone ไว้ตลอด (กันเคสโดนเตะ/ตกแมพ/respawn
--   แล้วโผล่นอกโซน) — ไม่ใช่ teleport ครั้งเดียวจบ
--==================================================================
function Auto:SafeZoneLoop()
  local function stillOn() return self.state.safeZone end
  ...
  while stillOn() do
    ...
    if hrp then
      local target = CFrame.new(self.SAFE_ZONE_POS)
      -- ขยับใหม่เฉพาะตอนเคลื่อนออกนอกโซนไปไกลพอ กันสั่น/กระตุกทุกเฟรม
      if (hrp.Position - self.SAFE_ZONE_POS).Magnitude > 4 then
...
-- ---- Go To Safe Zone ----
local safeZoneRow = titledRow(RerollForm, "Go To Safe Zone",
  "Teleport and hold at the safe zone")

Auto.toggles.GoToSafeZone = safeZoneRow:Right():Toggle({
  Value = false,
  ValueChanged = function(self, value)
    if Config.syncing then return end
    Auto.state.safeZone = value
    if value then
      Auto:RunThread("safeZone", function() Auto:SafeZoneLoop() end)
    else
      Auto:KillThread("safeZone")
    end
    Config:Request()
  end,
})

Config:Register("GoToSafeZone",
  function() return Auto.toggles.GoToSafeZone.Value end,
  function(v) Auto.toggles.GoToSafeZone.Value = v end
)
...
do
  ...
  local setDescButton = titledRow(miscForm, "Set Description", "Write your stands into the RAM account description")
...
resumeIfOn("GoToSafeZone", "safeZone", "safeZone",
  function() Auto:SafeZoneLoop() end)
